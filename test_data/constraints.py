...
def q95_input(r, a, bt, ip, kappa, delta):
    ...
    return (q95 >= 3.0) & (q95 <= 7.0)

def q95(q95_value):
    return (q95_value >= 3.0) & (q95_value <= 7.0)
...
def ibs_limits(ibs):
    return (ibs >= 0.0) & (ibs <= 12.0)

def inb_limits(inb):
    return (inb >= 0.0) & (inb <= 12.0)

def irf_limits(irf):
    return (irf >= 0.0) & (irf <= 12.0)

def zeff_axis_limits(zeff_axis):
    return (zeff_axis >= 1.0) & (zeff_axis <= 10.0)

def pfuse_limits(pfuse):
    return (pfuse >= 0.0) & (pfuse <= 20.0)

def pfusi_limits(pfusi):
    return (pfusi >= 0.0) & (pfusi <= 20.0)

def prfe_limits(prfe):
    return (prfe >= 0.0) & (prfe <= 20.0)

def prfi_limits(prfi):
    return (prfi >= 0.0) & (prfi <= 20.0)

def pnbe_limits(pnbe):
    return (pnbe >= 0.0) & (pnbe <= 20.0)

def pnbi_limits(pnbi):
    return (pnbi >= 0.0) & (pnbi <= 20.0)

def prad_limits(prad):
    return (prad >= 0.0) & (prad <= 20.0)

def tau98_limits(tau98):
    return (tau98 >= 0.0) & (tau98 <= 2.0)
